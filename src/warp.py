# From https://github.com/sunniesuhyoung/DST/blob/master/warp.py
...
def solve_interpolation(train_points, train_values, device):
    ...
    matrix_a = phi(pairwise_squared_distance_matrix(c)).to(device)  # [n, n]
    #matrix_a += torch.randn_like(matrix_a)*1e-3

    # Append ones to the feature values for the bias term in the linear model.
    ones = torch.ones([n, 1]).to(device)
    matrix_b = torch.cat([c.to(device), ones], 1)  # [n, d+1]
    ...
    rhs = torch.cat([f.to(device), rhs_zeros], 0)  # [n+d+1, k]
...
def apply_interpolation(query_points, train_points, w, v, device):
    ...
    pairwise_dists = cross_squared_distance_matrix(query_points.to(device), train_points.to(device), device)
